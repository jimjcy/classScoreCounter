...
# class_info = {}
# try:
#     with open(r'C:\Users\{}\.class\class_info.json'.format(user_name), mode='r', encoding='utf-8') as f:
#         class_info = eval(f.read())
#         f.close()
#     time = ti.localtime()
#     if class_info['time'] != time[2]:
#         raise Exception
# except Exception:
#     try:
#         if not o.path.exists(r'C:\Users\{}\.class\class_info.json'.format(user_name)):
#             raise Exception
#         if not o.path.exists(r'C:\Users\{}\.class'.format(user_name)):
#             o.mkdir(r'C:\Users\{}\.class'.format(user_name))
#         with open(r'C:\Users\{}\.class\class_info.json'.format(user_name), mode='r', encoding='utf-8') as f:
#             f.close()
#         class_info_old = class_info.copy()
...
#                 pass
#             else:
#                 class_info[str(num)] = {'总分': class_info_old[str(num)]['总分'], '加减分数': 0, '今日加减分数原因': []}
#         time = ti.localtime()
#         class_info['time'] = time[2]
#         with open(file=r'C:\Users\{}\.class\class_info.json'.format(user_name), mode='w', encoding='utf-8') as f:
#             f.write(str(class_info))
#             f.close()
#         with open(file=rf'C:\Users\{user_name}\.class\{time[0]}年{time[1]}月{time[2]}日 导出文件.txt', mode='a+', encoding='utf-8') as f:
...
#                         str(class_info[nums]['总分']), str(class_info[nums]['加减分数']),
#                         str(class_info[nums]['今日加减分数原因'])))
#             f.close()
#     except Exception:
#         if not o.path.exists(r'C:\Users\{}\.class'.format(user_name)):
#             o.mkdir(r'C:\Users\{}\.class'.format(user_name))
...
#                 pass
#             else:
#                 class_info[str(num)] = {'总分': 100, '加减分数': 0, '今日加减分数原因': []}
#         time = ti.localtime()
#         class_info['time'] = time[2]
#         with open(file=r'C:\Users\{}\.class\class_info.json'.format(user_name), mode='w', encoding='utf-8') as f:
#             f.write(str(class_info))
...
class main_window:
    def __init__(self, master: Tk) -> None:
        ...
        label1 = Label(self.win, text='班级登分程序')
        label1.pack()

        button1 = Button(self.win, text='加分', command=self.add)
        button1.place(relx=0.2, rely=0.3, relwidth=0.2)

        button2 = Button(self.win, text='减分', command=self.delete)
        button2.place(relx=0.6, rely=0.3, relwidth=0.2)

        button3 = Button(self.win, text='直接修改', command=self.write)
        button3.place(relx=0.2, rely=0.5, relwidth=0.2)

        button4 = Button(self.win, text='查询', command=self.search)
        button4.place(relx=0.6, rely=0.5, relwidth=0.2)

        button5 = Button(self.win, text='排行榜', command=self.rank)
        button5.place(relx=0.2, rely=0.7, relwidth=0.2)

        button6 = Button(self.win, text='导出文件', command=self.out)
        button6.place(relx=0.6, rely=0.7, relwidth=0.2)
